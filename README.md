...
| 因子开关 | `USE_RS` / `USE_ORB` / `USE_VWAP_SIGNAL` / `USE_ORDER_FLOW` / `USE_DARK_POOL_PROXY` / `USE_ORDER_BOOK_IMBALANCE` / `USE_L2_IMBALANCE_TRACKER` / `USE_INTRADAY_FLOW` / `USE_SHORT_METRICS` / `USE_OPTION_IV` |
...
本版本聚焦双市场研究、日报自动化与 OpenD 请求稳定性，不自动调整实盘权重，不改变实盘启动红线。

| 方向 | 变更 |
|---|---|
| SDK | 文档和安装指引升级到本地 `MMAPI4Python_10.7.6708/`；SDK 源码目录继续通过 `.gitignore` 排除，不纳入仓库 |
| 双市场 | `us_strategy/` 与 `hk_strategy/` 保持 US/HK 对称，新增/同步港股特化、watchlist、回测与测试覆盖 |
| 研究 CLI | 新增 `research.signal_lab`，支持 IC 显著性诊断、walk-forward、Optuna、QuantStats 与 vectorbt 独立研究扫描 |
| 美股日报 | 新增仓库本地技能和 `tools.run_us_stock_daily_report`，输出全文报告、结构化结果、期权摘要，并用飞书卡片摘要 + 云文档链接发送 |
| 因子增强 | 新增微观结构、盘口压力、L2 imbalance、dark-pool proxy、宏观/加密代理、期权诊断等研究因子；新增因子默认关闭、权重 0 |
| 限频 | 新增 `moomoo_rate_limits.py`，统一记录 moomoo 官方/保守限频；日报期权链和历史回填默认按官方节奏限速 |
| 验证 | 当前双市场测试集 `python -m pytest us_strategy\tests hk_strategy\tests -q` 通过 253 项 |

## 数据持久化

SQLite（默认 `us_strategy/positions.db` / `hk_strategy/positions.db`，已 gitignore）：
