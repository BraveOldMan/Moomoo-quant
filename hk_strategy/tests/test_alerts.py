...
from hk_strategy import alerts
from hk_strategy.alerts import AlertManager
...
def test_feishu_alert_sends_interactive_card(monkeypatch) -> None:
    ...
    manager.send("买入成功", "HK.00700 成交均价=400.000 qty=100")
    ...
    assert card["header"]["title"]["content"] == "港股策略提醒 - 买入成功"
...
def test_no_feishu_chat_id_does_not_call_lark(monkeypatch) -> None:
    ...
    AlertManager(StrategyConfig()).send("买入成功", "HK.00700")
