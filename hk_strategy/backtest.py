...
使用示例：
    ...
    result = engine.run(["HK.00700", "HK.09988"], start="2024-01-01", end="2024-06-30")
    print(result.report())
"""
...
class BacktestEngine:
    ...
    def _apply_decision(
        ...
        if in_pos and cfg.use_trailing_stop:
            ...
            if close > peak:
                pos["peak"] = peak = close
            if can_exit and peak > 0 and (peak - close) / peak >= cfg.trailing_stop_pct:
    ...
    # ── 成本模型（港股按成交额计费：佣金 + 印花税 + 交易所/证监费）──────
    def _fees(self, value: float) -> float:
        """单边费用合计 = max(最低佣金, 成交额×佣金率) + 印花税 + 交易所等杂费。"""
        cfg = self._cfg
        commission = max(cfg.commission_min, value * cfg.commission_pct)
        levies = value * (cfg.stamp_duty_pct + cfg.exchange_fees_pct)
        return commission + levies

    def _gross_cost(self, qty: int, price: float) -> tuple[float, float]:
        """买入总支出 = 数量×(价×(1+滑点)) + 单边费用。"""
        slip = self._cfg.slippage_bps / 10_000.0
        value = qty * price * (1 + slip)
        fees = self._fees(value)
        return value + fees, fees

    def _net_proceeds(self, qty: int, price: float) -> tuple[float, float]:
        """卖出净回款 = 数量×(价×(1-滑点)) - 单边费用。"""
        slip = self._cfg.slippage_bps / 10_000.0
        value = qty * price * (1 - slip)
        fees = self._fees(value)
        return value - fees, fees
