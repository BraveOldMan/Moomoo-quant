...
#
# Runs `python -m hk_strategy.forward_monitor`, which scores the watchlist on a
# loop and persists every factor score into signal_log WITHOUT placing orders.
# The monitor self-gates HKEX hours incl. the lunch break (skips rounds when
# closed), so this launcher only needs a generous daily window.
#
# Registered by: Register-ScheduledTask "MoomooHKForwardCollect"
#   Trigger : weekly Mon-Fri 09:15 (Beijing = HKT, no DST) — before 09:30 open
#   Limit   : ~7.5h run window -> stops ~16:45 Beijing, after the 16:00 close
#
...
$log  = Join-Path $root 'hk_strategy\forward_monitor.log'
Set-Location $root
...
& cmd /c "`"$py`" -u -m hk_strategy.forward_monitor >> `"$log`" 2>&1"
