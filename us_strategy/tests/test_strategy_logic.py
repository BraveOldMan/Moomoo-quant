...
from us_strategy.signals import SignalResult
...
def test_trading_days_between_skips_holiday():
    # 2024-07-03 周三 → 2024-07-05 周五，跨独立日(7/4休市) = 1 个交易日
    assert _trading_days_between(date(2024, 7, 3), date(2024, 7, 5)) == 1
...
def test_buy_gate_blocks_only_new_buy_signal():
    result = _signal_result(buy_block_reasons=["纳指/VIX宏观风险偏高"])
    s = _strategy_with_result(result, min_hold_days=0)

    decision = s.evaluate("US.X", current_price=10.0)
...
def test_buy_gate_does_not_block_sell_signal():
    result = _signal_result(
        ...
        buy_block_reasons=["纳指/VIX宏观风险偏高"],
    )
    s = _strategy_with_result(result, min_hold_days=0)
    s.record_buy("US.X", 10.0, 100)
...
def test_ipo_position_uses_ipo_take_profit():
    result = _signal_result(composite_score=20.0)
    s = _strategy_with_result(result, min_hold_days=0, ipo_take_profit_pct=0.12)
    s.record_buy("US.X", 10.0, 100, origin="ipo")

    decision = s.evaluate("US.X", current_price=11.3)
...
def test_regular_position_does_not_use_ipo_take_profit():
    result = _signal_result(composite_score=20.0)
    s = _strategy_with_result(result, min_hold_days=0, ipo_take_profit_pct=0.12)
    s.record_buy("US.X", 10.0, 100, origin="regular")
