...
def test_once_event_gate_dedupes_ipo_key_events() -> None:
    ...
    assert gate.should_send("发现今日IPO", "US.WHK") is True
    assert gate.should_send("发现今日IPO", "US.WHK") is False
    assert gate.should_send("IPO首次分析", "US.WHK") is True
    assert gate.should_send("发现今日IPO", "US.NEXT") is True
