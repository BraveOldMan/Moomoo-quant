...
> 验证：`pytest us_strategy hk_strategy tools` 420 项全绿 · `ruff check` 通过 · US+HK 同步

对"回测策略"与"美股日报技能"是否与实盘主策略（`signals.py`→`features.py`→`strategy.py`）保持一致做了系统核查（逐条回源码验证，剔除代理误报）。

**结论：美股日报忠实复用实盘**——`analyze_stocks` 用 `SignalCalculator`+`IPOStrategy.evaluate()`，signal/score/reason 直接取自 `decision`；诊断配置仅经独立 `diag_calc` 旁路展示，不污染主决策；期权风险分复用 `features.iv_skew_score/pcr_score`。无需改动。

**回测发现并已对齐的不一致：**

1. **capital 因子（权重最高 0.55）**：实盘用 `capital_outflow_score`（机构流出/总额比，`get_capital_distribution` 快照，无历史），美股不可用时**丢弃**该因子并重新归一化；回测/分析原先**始终注入** `capital_flow_score`（美股 kline 无 `main_in_flow` → 恒中性 50），稀释综合分。
   - **修复**：回测/分析仅在确有真实逐根资金流数据时计入 capital，否则丢弃，使因子存在性与实盘一致。`capital_flow_score` docstring 已澄清其为历史代理、口径不同。
2. **组合熔断**：实盘有日内亏损熔断（`check_and_update_circuit_breaker`），回测原先缺失。
   - **修复**：回测主循环按交易日复刻——当日组合较上一交易日回撤 ≥ `daily_loss_limit_pct` 时阻断当日新开/加仓（已持仓止损止盈不受影响）。
3. **turnover_rate 单位**：回测 kline 值 ×100 对齐 snapshot 百分数口径，已加注释固化该假设。

**因子可回测性清单（"测的就是跑的"边界）：**

| 类别 | 因子 | 历史回测/IC | 校准方式 |
|---|---|---|---|
| 可历史回放 | turnover / momentum / rs | ✅ 同函数同窗口 | 历史 IC |
| 代理（口径不同） | capital | ⚠️ 仅有真实 `main_in_flow` 时；为净流强度代理 | **前向 IC**（`forward_ic_from_log`） |
| 优雅降级一致 | short | ✅ == 实盘"仅 daily_short_volume"分支 | 历史 IC（dtc 增量须前向） |
| 无历史回放 | orb / vwap / order_flow / obi / l2 / option_iv | ❌ 回测不计 | **前向 IC**（须实盘日志） |

> HK 平行包 `hk_strategy` 已同步全部对齐（backtest/analysis/features）。港股扩展因子须在港股样本独立校准，不沿用美股结论。
